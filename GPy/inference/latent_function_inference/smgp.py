...
class SMGP(LatentFunctionInference):
	...
	def inference(self, q_u_mean, q_u_chol, kern, Z, likelihood,  mean_function,Y_metadata, l_svgps, X, Y, rho,  KL_scale=1.0, batch_scale=1.0):
		...
		l_posteriors=[];
		l_grad_dicts =[];
		dF_dmu_full= np.empty((num_data,1));
		dF_dv_full= np.empty((num_data,1));
		for i in range(len(l_svgps)):
			l_posterior, l_log_marginal, l_grad_dict,index, dF_dmu, dF_dv = self.local_inference(X, Y, rho[:, i], l_svgps[i].q_u_mean, l_svgps[i].q_u_chol, l_svgps[i].kern, l_svgps[i].Z, l_svgps[i].likelihood,  l_svgps[i].mean_function,  g_mu, g_v,  l_svgps[i].Y_metadata, KL_scale, batch_scale)
			...
			dF_dmu_full[index,:]=dF_dmu
			dF_dv_full[index,:]=dF_dv


#            g_grad_dict['dL_dKmm']+=add_g_grad_dict['dL_dKmm']
#            g_grad_dict['dL_dKmn'][:, index]+=add_g_grad_dict['dL_dKmn']
#            #g_grad_dict['dL_dKdiag']=g_grad_dict['dL_dKdiag']+add_g_grad_dict['dL_dKdiag']
#            g_grad_dict['dL_dm']+=add_g_grad_dict['dL_dm']
#            g_grad_dict['dL_dchol']+=add_g_grad_dict['dL_dchol']
#            #g_grad_dict['dL_dthetaL']=g_grad_dict['dL_dthetaL']+add_g_grad_dict['dL_dthetaL']


			#g_grad_dict = self.addDicts(g_grad_dict, add_g_grad_dict)

		add_g_grad_dict=self.derivative_wrt_g(num_data, num_outputs, dF_dmu_full, dF_dv_full, g_A, g_L, g_S, g_Kmmi,g_Kmmim)
		g_grad_dict['dL_dKmm']+=add_g_grad_dict['dL_dKmm']
		g_grad_dict['dL_dKmn']+=add_g_grad_dict['dL_dKmn']
		#g_grad_dict['dL_dKdiag']=g_grad_dict['dL_dKdiag']+add_g_grad_dict['dL_dKdiag']
		g_grad_dict['dL_dm']+=add_g_grad_dict['dL_dm']
		g_grad_dict['dL_dchol']+=add_g_grad_dict['dL_dchol']
		#g_grad_dict['dL_dthetaL']=g_grad_dict['dL_dthetaL']+add_g_grad_dict['dL_dthetaL']
		...
		return g_posterior, l_posteriors, g_log_marginal_likelihood, g_grad_dict, l_grad_dicts
...
#        return Posterior(mean=q_u_mean, cov=S.T, K=Kmm, prior_mean=prior_mean_u), log_marginal, grad_dict, g_grad_dict
	...
	def global_inference(self, q_u_mean, q_u_chol, kern, X, Z, likelihood, Y, mean_function=None, Y_metadata=None, KL_scale=1.0, batch_scale=1.0):
		...
		if np.any(np.isinf(Si)):
			L1,jitter =linalg.jitchol2(S[0,:,:]);
			L[0,:,:]=L1;
			S[0,:,:]=S[0,:,:] + np.eye(S[0,:,:].shape[0]) * jitter;
		...
		if mean_function is not None:
			grad_dict['dL_dmfZ'] = dF_dmfZ - dKL_dmfZ
			grad_dict['dL_dmfX'] = dF_dmfX
		return Posterior_global(mean=q_u_mean, cov=S.T, K=Kmm, prior_mean=prior_mean_u), log_marginal, grad_dict, mu,partial_v, A, L, S, Kmmi, Kmmim

	#@profile
	def local_inference(self,  X_full, Y_full, rho_full, q_u_mean, q_u_chol, kern, Z, likelihood,  mean_function,  g_mu_full, g_v_full,  Y_metadata=None, KL_scale=1.0, batch_scale=1.0):

		#index = np.where(rho_full>0)[0]
		#index = (rho_full>0).flatten()
		index = rho_full   #use this for the case of MAP expert allocation
		X = X_full[index, :]
		Y = Y_full[index, :]
		#rho = rho_full [index, :]
		rho = rho_full [index,None]
		...
		return Posterior(mean=q_u_mean, cov=S.T, K=Kmm, prior_mean=prior_mean_u), log_marginal, grad_dict,index, dF_dmu, dF_dv


	#@profile
	def local_inference_multithread(self,  X_full, Y_full, rho_full, lsvgp, g_mu_full, g_v_full,  KL_scale=1.0, batch_scale=1.0):
		...
		#quadrature for the likelihood
		F, dF_dmu, dF_dv, dF_dthetaL = lsvgp.likelihood.variational_expectations(Y, mu, v, Y_metadata=lsvgp.Y_metadata)
	...
	def derivative_wrt_g(self, num_data, num_outputs, dF_dmu, dF_dv, g_A, g_L, g_S, g_Kmmi,g_Kmmim):
		...
		dL_dgchol = 2.*np.array([np.dot(a,b) for a, b in zip(dL_dgS, g_L) ])
		#dL_dgchol = choleskies.triang_to_flat(dL_dgchol)
		g_grad_dict = {'dL_dKmm':dL_dgKmm, 'dL_dKmn':dL_dgKmn, 'dL_dKdiag': 0, 'dL_dm':dL_dgm, 'dL_dchol':dL_dgchol, 'dL_dthetaL':0}
		return g_grad_dict
