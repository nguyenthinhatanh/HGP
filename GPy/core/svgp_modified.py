...
class SVGPM(SparseGP):
    def __init__(self, X, Y, Z, kernel, likelihood, mean_function=None, name='SVGPM', Y_metadata=None, batchsize=None, num_latent_functions=None):
        """
...
#            X_batch, Y_batch = self.new_batch()
        ...
        super(SVGPM, self).__init__(X, Y, Z, kernel, likelihood, mean_function=mean_function, inference_method=inf_method,
    ...
    def update_gradients_helper(self, grad_dictionary):
        #update the kernel gradients
        self.kern.update_gradients_full(grad_dictionary['dL_dKmm'], self.Z)
        grad = self.kern.gradient.copy()
        self.kern.update_gradients_full(grad_dictionary['dL_dKmn'], self.Z, self.X)
        grad += self.kern.gradient.copy()
        self.kern.update_gradients_diag(grad_dictionary['dL_dKdiag'], self.X)
        self.kern.gradient += grad
        if not self.Z.is_fixed:# only compute these expensive gradients if we need them
            self.Z.gradient = self.kern.gradients_X(grad_dictionary['dL_dKmm'], self.Z) + self.kern.gradients_X(grad_dictionary['dL_dKmn'], self.Z, self.X)


        self.likelihood.update_gradients(grad_dictionary['dL_dthetaL'])
        #update the variational parameter gradients:
        self.m.gradient = grad_dictionary['dL_dm']
        self.chol.gradient = memoryview(grad_dictionary['dL_dchol'])

        if self.mean_function is not None:
            self.mean_function.update_gradients(grad_dictionary['dL_dmfX'], self.X)
            g = self.mean_function.gradient[:].copy()
            self.mean_function.update_gradients(grad_dictionary['dL_dmfZ'], self.Z)
            self.mean_function.gradient[:] += g
            self.Z.gradient[:] += self.mean_function.gradients_X(grad_dictionary['dL_dmfZ'], self.Z)
